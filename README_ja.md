...
> **免責事項**：本プラグインは**合法的な用途のみ**（チャット履歴のレイアウトデモ、コンテンツ制作、詐欺防止の啓発など）を目的としています。チャット履歴を偽装して詐欺、誹謗中傷、なりすまし、証拠偽造など、違法または不道徳な行為を行うことに利用することは**厳禁**です。本プラグインの使用によって生じるすべての法的・道徳的責任は利用者が自ら負い、作者およびプロジェクトは一切の責任を負いません。
...
```
/伪造消息 123456|こんにちは \| 654321|王さん|こちらこそ、よろしく
```
...
```
/伪造外表 123456|ミン|今夜、ご飯でもどう? \| 654321|ホン|いいね、7時に待ち合わせ \\| 秘密の会話
```

外側には「秘密の会話」と表示され、中身はミンとホンのチャット履歴です。
...
- 英語エイリアス：/fake-message · /fake-appearance · /fake-help
...
```
ユーザー: ミンとのチャット履歴を偽装して。私が「また明日」、ミンが「うん、また明日」
🤖 → create_forward(params={"segments":[{"qq":"123456","text":"また明日"},{"qq":"654321","nickname":"ミン","text":"うん、また明日"}]})
    まとめ転送を送信しました（2 件）✅
```
...
{
  "segments": [
    {"qq": "123456", "text": "こんにちは", "nickname": "ワン", "time": 1756684800, "image": "url"}
  ],
  "title": "任意の外側タイトル"
}
```
...
> **ヒント：**パラメーターの渡し方は 2 種類あります：`{"segments": [...]}` 形式と、配列 `[...]` を直接渡す方法です。
